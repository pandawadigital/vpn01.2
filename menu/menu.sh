...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/pandawadigital/permission/main/ipmini > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/pandawadigital/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/pandawadigital/permission/main/ipmini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/pandawadigital/permission/main/ipmini | grep $MYIP | awk '{print $3}')
fi
...
export sem=$( curl -s https://raw.githubusercontent.com/pandawadigital/permission/main/versions)
...
echo -e "${BIYellow}□ AutoScript By Dev   = Wak ${GREEN}( PandawaDigital )${NC}"

echo -e "${BICyan} ┌─────────────────────────────────────────────────────┐${NC}"
echo -e "${BICyan} │                  ${BIWhite}${UWhite}PANDAWADIGITAL TUNELING${NC}"
echo -e "${BICyan} │"
...
13) clear ; slow ;;
99) clear ; clearcache ;;
6969) clear ; wget https://raw.githubusercontent.com/pandawadigital/vpn01.2/main/update.sh && chmod +x update.sh && ./update.sh && rm -f /root/update.sh ;;
0) clear ; menu ;;
x) exit 
